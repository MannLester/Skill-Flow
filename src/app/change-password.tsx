...
export default function ChangePasswordScreen() {
  ...
  return <MobilePage backgroundColor={authColors.background}><StatusBar style="light" /><AppHeader title="Change Password" onBack={() => router.back()} /><KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><ScrollView contentContainerStyle={styles.content}><AppText style={styles.note}>Clerk securely manages your password. SkillFlow never stores it locally.</AppText><FormField light icon="lock-closed-outline" value={current} onChangeText={(value) => { setCurrent(value); setError(''); }} placeholder="Current password" secureTextEntry /><FormField light icon="key-outline" value={next} onChangeText={(value) => { setNext(value); setError(''); }} placeholder="New password" secureTextEntry /><FormField light icon="checkmark-circle-outline" value={confirm} onChangeText={(value) => { setConfirm(value); setError(''); }} placeholder="Confirm new password" secureTextEntry />{error ? <AppText accessibilityRole="alert" style={styles.error}>{error}</AppText> : null}<PrimaryButton title={busy ? 'Updating…' : 'Update Password'} disabled={busy} onPress={save} /></ScrollView></KeyboardAvoidingView></MobilePage>;
}
const styles = StyleSheet.create({ content: { flexGrow: 1, gap: 12, padding: contentPadding, justifyContent: 'center' }, note: { color: authColors.muted, fontSize: 11, textAlign: 'center', marginBottom: 8 }, error: { color: colors.red, fontSize: 10, textAlign: 'center' } });
