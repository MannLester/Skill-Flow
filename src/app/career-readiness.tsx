...
import { AppHeader, AppText, HeroDecor, MobilePage } from '@/components/ui';
...
export default function CareerReadinessScreen() {
  ...
  return <MobilePage><StatusBar style="light" /><AppHeader title="Career Readiness" onBack={() => router.back()} />
    <ScrollView contentContainerStyle={styles.content}>
      <View style={styles.scoreCard}><HeroDecor /><ScoreRing score={breakdown.score} /><View style={{ flex: 1 }}><AppText weight="bold" style={styles.level}>{breakdown.level}</AppText><AppText style={styles.intro}>Your score updates automatically from your persisted SkillFlow demo activity.</AppText></View></View>
...
function ScoreRing({ score }: { score: number }) {
  ...
  return <View style={{ width: SIZE, height: SIZE, alignItems: 'center', justifyContent: 'center' }}><Svg width={SIZE} height={SIZE}><Circle cx={CENTER} cy={CENTER} r={RADIUS} stroke={colors.green} strokeWidth={STROKE} fill="none" /><Circle cx={CENTER} cy={CENTER} r={RADIUS} stroke={colors.white} strokeWidth={STROKE} fill="none" strokeDasharray={CIRCUMFERENCE} strokeDashoffset={CIRCUMFERENCE * (1 - progress)} strokeLinecap="round" rotation="-90" origin={`${CENTER}, ${CENTER}`} /></Svg><View style={{ position: 'absolute', alignItems: 'center' }}><AppText weight="bold" style={{ color: colors.white, fontSize: 36, lineHeight: 39 }}>{score}</AppText><AppText style={{ color: colors.white, fontSize: 10 }}>/100</AppText></View></View>;
}
...
const styles = StyleSheet.create({
  ...
  scoreCard: { minHeight: 150, backgroundColor: colors.red, borderRadius: 18, padding: 19, flexDirection: 'row', alignItems: 'center', gap: 18, overflow: 'hidden', elevation: 6, shadowColor: colors.deepRed, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 10 }, level: { color: colors.white, fontSize: 21 }, intro: { color: colors.white, fontSize: 10, lineHeight: 16, marginTop: 5 },
  notice: { flexDirection: 'row', alignItems: 'center', gap: 9, backgroundColor: colors.blush, borderRadius: 11, padding: 12 }, noticeText: { flex: 1, color: colors.burgundy, fontSize: 9, lineHeight: 15 }, heading: { fontSize: 19, marginTop: 8 },
  category: { backgroundColor: colors.background, borderRadius: 14, padding: 15, ...shadow }, categoryTop: { flexDirection: 'row', alignItems: 'center', gap: 10 }, categoryIcon: { width: 42, height: 42, borderRadius: 21, backgroundColor: colors.blush, alignItems: 'center', justifyContent: 'center' }, categoryTitle: { fontSize: 13 }, detail: { color: colors.muted, fontSize: 9, marginTop: 2 }, points: { color: colors.burgundy, fontSize: 15 }, track: { height: 7, backgroundColor: colors.border, borderRadius: 4, overflow: 'hidden', marginTop: 13 }, fill: { height: '100%', backgroundColor: colors.red, borderRadius: 4 }, next: { flexDirection: 'row', alignItems: 'center', marginTop: 11 }, nextText: { flex: 1, color: colors.muted, fontSize: 9, lineHeight: 14 },
