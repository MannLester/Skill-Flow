...
import { AppText, PrimaryButton, QuickAction } from '@/components/ui';
...
function RecentProjectCard({ booking }: { booking?: ProjectBooking }) {
  return <Pressable disabled={!booking} onPress={() => booking && router.push({ pathname: '/projects/[projectId]', params: { projectId: booking.id } })} style={styles.projectCard}>
    {booking ? <OptimizedArtwork source={optimizedArtwork.studentProject} style={styles.projectImage} /> : null}
    {booking ? <View style={{ flex: 1 }}><AppText weight="semibold" style={styles.projectTitle}>{booking.title}</AppText><AppText weight="semibold" style={styles.projectPrice}>{formatPeso(booking.budget)}</AppText></View> : <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', gap: 8 }}><AppText weight="semibold" style={styles.emptyProjectTitle}>No project requests yet</AppText><PrimaryButton title="Browse Projects" onPress={() => router.push('/projects/discover')} style={{ minHeight: 32, minWidth: 120, borderRadius: 6 }} textStyle={{ fontSize: 12 }} /></View>}
...
const styles = StyleSheet.create({
  ...
  projectCard: { backgroundColor: colors.background, borderRadius: 14, minHeight: 128, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 14, ...shadow }, projectImage: { width: 92, aspectRatio: 122 / 117, borderRadius: 11 }, projectTitle: { fontSize: 16, lineHeight: 21 }, projectPrice: { marginTop: 10, fontSize: 15 },   emptyProjectHint: { color: colors.burgundy, fontSize: 12, marginTop: 4 }, emptyProjectTitle: { fontSize: 14, lineHeight: 21 }, statusPill: { backgroundColor: colors.blush, borderRadius: 14, paddingHorizontal: 12, paddingVertical: 8 }, statusText: { color: colors.burgundy, fontSize: 11 },
  recommendCard: { flexDirection: 'row', alignItems: 'center', gap: 14, borderRadius: 14, backgroundColor: colors.background, padding: 12, ...shadow }, recommendImage: { width: 72, height: 62, borderRadius: 10, backgroundColor: colors.blush, alignItems: 'center', justifyContent: 'center' },
});
