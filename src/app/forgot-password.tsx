...
function RecoveryFields({ step, values, setters, clearError }: RecoveryFieldsProps) {
  if (step === 'email') return <FormField light icon="mail-outline" placeholder="Email Address" value={values.email} onChangeText={(value) => { setters.setEmail(value); clearError(); }} keyboardType="email-address" autoCapitalize="none" />;
  if (step === 'code') return <FormField light icon="key-outline" placeholder="Recovery Code" value={values.code} onChangeText={(value) => { setters.setCode(value); clearError(); }} keyboardType="number-pad" />;
  return <><FormField light icon="lock-closed-outline" placeholder="New Password" value={values.password} onChangeText={(value) => { setters.setPassword(value); clearError(); }} secureTextEntry /><FormField light icon="checkmark-circle-outline" placeholder="Confirm Password" value={values.confirm} onChangeText={(value) => { setters.setConfirm(value); clearError(); }} secureTextEntry /></>;
}

const styles = StyleSheet.create({ content: { flexGrow: 1, justifyContent: 'center', gap: 14, paddingHorizontal: contentPadding, paddingBottom: 40 }, copy: { color: authColors.muted, fontSize: 12, lineHeight: 18, textAlign: 'center', marginBottom: 8 }, error: { color: colors.red, fontSize: 10, textAlign: 'center' } });
