...
import { ProjectMessage, useSession } from '@/context/session';
  ...
  return (
    <MobilePage>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <FlatList data={thread} keyExtractor={(item) => item.id} renderItem={({ item }) => <Bubble message={item} own={item.senderId === currentAccount?.id} sender={accounts.find((account) => account.id === item.senderId)?.name ?? 'User'} />} contentContainerStyle={styles.list} ListEmptyComponent={<View style={styles.empty}><AppText weight="semibold">Start the conversation</AppText><AppText style={styles.emptyText}>Messages are stored locally with this demonstration project.</AppText></View>} />
