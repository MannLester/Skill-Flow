...
export default function RegisterScreen() {
  ...
  const start = async () => {
    ...
    setError('');
    const created = await signUp.create({ emailAddress: form.email.trim().toLowerCase(), password: form.password, legalAccepted: true, unsafeMetadata: accountMetadata(form.name, role) });
...
function SocialAuthButtons({ busy, disabled, onSelect }: { busy: OAuthProvider | null; disabled: boolean; onSelect: (provider: OAuthProvider) => void }) {
  return <View style={styles.socialBlock}><View style={styles.dividerRow}><View style={styles.divider} /><AppText style={styles.dividerText}>or</AppText><View style={styles.divider} /></View><SocialButton provider="google" title={busy === 'google' ? 'Connecting to Google...' : 'Continue with Google'} disabled={disabled} onPress={() => onSelect('google')} /><SocialButton provider="facebook" title={busy === 'facebook' ? 'Connecting to Facebook...' : 'Continue with Facebook'} disabled={disabled} onPress={() => onSelect('facebook')} /></View>;
}

function SocialButton({ disabled, onPress, provider, title }: { disabled: boolean; onPress: () => void; provider: OAuthProvider; title: string }) {
  const color = provider === 'google' ? '#db4437' : '#1877f2';
  return <Pressable accessibilityRole="button" accessibilityLabel={title} disabled={disabled} onPress={onPress} style={({ pressed }) => [styles.socialButton, disabled ? styles.disabled : null, pressed && !disabled ? styles.pressed : null]}><Ionicons name={`logo-${provider}`} size={20} color={color} /><AppText weight="semibold" style={styles.socialText}>{title}</AppText></Pressable>;
}
...
function accountMetadata(name: string, role: UserRole) { return { skillflowName: name.trim() || undefined, skillflowRole: role }; }
function labelFor(provider: OAuthProvider) { return provider === 'google' ? 'Google' : 'Facebook'; }
...
const styles = StyleSheet.create({
  ...
  terms: { flexDirection: 'row', alignItems: 'flex-start', gap: 8, marginTop: 16 },
  ...
  error: { color: colors.red, fontSize: 10, textAlign: 'center', marginTop: 12 },
  socialBlock: { gap: 10, marginTop: 16 },
  dividerRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  divider: { flex: 1, height: 1, backgroundColor: colors.border },
  dividerText: { color: colors.muted, fontSize: 10 },
  socialButton: { minHeight: 48, borderRadius: 8, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.white, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10 },
