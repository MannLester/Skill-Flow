...
export default function DiscoverProjectsScreen() {
  ...
  return <MobilePage><StatusBar style="light" /><AppHeader title="Discover Projects" onBack={() => router.back()} />
    ...
    <FlatList horizontal data={categories} keyExtractor={(item) => item} showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filters} renderItem={({ item }) => <Pressable onPress={() => setCategory(item)} style={[styles.chip, category === item && styles.chipActive]}><AppText weight="medium" style={[styles.chipText, category === item && styles.chipTextActive]}>{item}</AppText></Pressable>} />
...
const styles = StyleSheet.create({
  ...
  filters: { paddingHorizontal: contentPadding, paddingVertical: 12, gap: 8 }, chip: { borderWidth: 1, borderColor: colors.border, borderRadius: 16, paddingHorizontal: 13, paddingVertical: 7 }, chipActive: { backgroundColor: colors.red, borderColor: colors.red }, chipText: { fontSize: 10, color: colors.muted }, chipTextActive: { color: colors.white },
