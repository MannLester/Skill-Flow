...
export default function ProjectsScreen() {
  ...
  return (
    <MobilePage>
      ...
      <BottomNav active="projects" onHome={() => router.replace(homeRoute)} onProjects={() => undefined} onPortfolio={currentAccount?.role === 'student' ? () => router.push('/portfolio') : undefined} onMessages={() => router.push('/messages')} onSaved={currentAccount?.role === 'client' ? () => router.push({ pathname: '/marketplace', params: { saved: 'true' } }) : undefined} onProfile={() => router.push('/profile')} variant={currentAccount?.role === 'client' ? 'client' : 'student'} />
    </MobilePage>
  );
}
