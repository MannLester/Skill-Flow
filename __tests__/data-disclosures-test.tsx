...
describe('cloud data-handling disclosures', () => {
  it('identifies cloud storage, Clerk processing, simulations, and available controls in the privacy notice', () => {
    ...
    expect(screen.getByText(/Student Verification accepts fictional sample data and automatically marks a complete demo submission approved/i)).toBeTruthy();
    expect(screen.getByText(/does not assess document authenticity or perform real identity verification/i)).toBeTruthy();
    expect(screen.getByText(/AI Mentor sends prompts to OpenCode Go/i)).toBeTruthy();
    expect(screen.getByText(/When the AI service is unavailable, unsent messages can be retried/i)).toBeTruthy();
  ...
  it('states the same material cloud and simulation boundaries in the terms', () => {
    ...
    expect(screen.getByText(/Ratings and in-app notifications are stored application records generated by demo activity/i)).toBeTruthy();
    expect(screen.getByText(/No real contract, payment, escrow, or transfer is created/i)).toBeTruthy();
    expect(screen.getByText(/mentor sends prompts to OpenCode Go/i)).toBeTruthy();
    expect(screen.getByText(/When the AI service is unavailable, unsent messages can be retried/i)).toBeTruthy();
  ...
  it('corrects cloud storage, recovery, mentor, verification, and reset answers in Help', () => {
    ...
    expect(screen.getByText(/prompts are sent to OpenCode Go/i)).toBeTruthy();
    expect(screen.getByText(/When the AI service is unavailable, unsent messages can be retried/i)).toBeTruthy();
